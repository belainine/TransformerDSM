...
class DotProductNormalise(nn.Module):
    def __init__(self):
        super().__init__()
        self.i=0
    def normalise(self,inputa,p=0):
...
class MultiHeadAttentionBSM(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_q, d_qi, dropout=0.1):
        ...
        self.attention = DotProductAttention(temperature=d_q ** 0.5,BSM=True)
        self.n=DotProductNormalise()
    ...
    def forward(self, q, k, qi, mask=None):
        ...
        q = self.w_qs(q).view(sz_b, -1, n_head, d_q)
        qi = self.w_qis(qi).view(sz_b, -1, n_head, d_qi)

        #q = self.w_qs(q).view(sz_b, len_q, n_head, d_q)
        #qi = self.w_qis(qi).view(sz_b, len_qi, n_head, d_qi)
        
        qi_n,donum1=self.n.normalise(qi,p=0)
        qi_n=qi_n.transpose(1,2)
        # Transpose for attention dot product: b x n x lq x dv
        q,donum2 = self.n.normalise(q)
        q=q.transpose(1,2)
        p=0
        if p==1:
            self.i+=1
            s=min(donum1.size(0),donum2.size(0))
            showPlot(donum2[0:s],rank=self.i,path='images/plot')    
            showPlot(donum1[0:s],rank=self.i,path='images/plot1')  
        if mask is not None:
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        self.attention = DotProductAttention(temperature=d_k ** 0.5, BSM=False)
